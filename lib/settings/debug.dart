...
class _DebugScreenState extends State<DebugScreen> {
  ...
  void didChangeDependencies() {
    ...
    _errorSubscription = repository.errorStream().listen((error) {
      if (!mounted) return;
      setState(() {
        _errorCount++;
        _lastError = error;
  ...
  Future<void> _pingDaemon() async {
    ...
    try {
      ...
      Log.e(_tag, 'Daemon ping failed: $error');
      if (mounted) {
        setState(() => _probeResult = 'daemon ping failed: $error');
      }
    }
  }

  Future<void> _querySessionPaused() async {
    ...
    try {
      ...
      Log.e(_tag, 'isSessionPaused failed: $error');
      if (mounted) {
        setState(() => _probeResult = 'isSessionPaused failed: $error');
      }
    }
  }
